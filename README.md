...
- **2026-09-13（3）** - Perk 优先级以刷取清单为准：三份刷取清单-紫枪的 Perk 三号位/四号位推荐（经别名词表归一化为 Manifest 名称，如「不稳定弹药→失衡弹药」「战壕→战嚎炮管」）排到对应特性列最前；购物清单缺失的刷取推荐 Perk 解析 hash 后补入列首；组合行数 8412 → 10539
- **2026-09-13（2）** - 合并刷取清单的 T 档评级、类型评级与评级理由至 216 把武器的 notes
- **2026-09-13** - 升级为购物清单全量版本：748 把武器（白弹 2946 + 绿弹 2090 + 威能 3131 + 其他 245），基于 Bungie 中文 Manifest 重新解析全部武器与 Perk hash；同步重建两个 hash 映射表；移除旧 CSV 源表（内容已被购物清单全量覆盖）
